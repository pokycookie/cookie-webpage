    ...
    <main className={styles.main}>
      <article>
        ...
        <section>
          ...
          <p>
            이번 프로젝트에서 가장 기술적으로 어려움을 겪었던 파트는 가장 가볍게
            생각했던 타이머 파트였습니다. 이전에도 현재 시간, 스톱워치, 타이머와
            같은 기능을 포함하는 애플리케이션은 제작해본 경험이 꽤 있었기 때문에
            크게 어렵게 생각하지 않았습니다.
          </p>
          <p>
          ...
          <p>
            ...
            하지만 실제로 탭을 최소화하거나, 다른 탭을 열어보는 등의 행동으로 웹
            애플리케이션이 작동하는 탭을 비활성화 한 결과, 미세하지만 초 단위로
            계속해서 오차가 누적되는 것을 확인할 수 있었습니다.
          </p>
          <p>
            따라서 이에 대해 자세히 알아본 결과, 자바스크립트 엔진은 사용자
            디바이스의 리소스를 최대한 효율적으로 사용하기 위해 비활성화 된 탭의
            활동에 스로틀링을 건다는 것을 알 수 있었습니다. 타이머, 오디오,
            비디오와 같은 API들이 이런 영향을 받을 수 있다고 했고, 제가 직면한
            문제도 이러한 이유임을 알 수 있었습니다.
          </p>
          <p>
            이를 해결하기 위해 여러 방법들을 찾아보았고, 그 결과 서비스워커라는
            기술에 대해 알게되었습니다. 서비스워커는 브라우저의 백그라운드에서
            실행되는 스크립트로, 탭의 비활성화 여부에 상관없이 타이머 함수를
            일정한 시간 간격으로 사용할 수 있게 할 수 있었습니다.
          </p>
          <p>
            마침 서비스워커를 사용하여 setInterval을 제공하는 worker-timmers라는
            라이브러리를 발견하였고, 기존에 사용하던 useInterval훅에
            setInterval을 해당 라이브러리 함수로 교체하였습니다. 예상대로
            비활성화 된 탭에서 스로틀링이 걸리는 현상이 원인이 맞았고,
            서비스워커를 사용함으로써 이 문제를 해결할 수 있었습니다.
          </p>
          ...
          <p>
            ...
            사용한 케이스만 있었기 때문에, 막상 단순 웹의 기능만 이용해서 파일을
            업로드하고 다운로드하게 만드려고 하니 어떻게 해야 할지 감이 잘
            잡히지 않았습니다.
          </p>
          <p>
          ...
          <p>
            input태그의 타입을 file로 지정하면 로컬 파일을 업로드하는데에 사용할
            수 있다는 것은 이전에도 알고 있었습니다. 하지만 a태그에서 download
            속성을 이용하면 해당 url의 콘텐츠를 다운받도록 할 수 있다는 것은
            이번 경험을 통해 처음 알게 되었습니다. download를 위한 파일은 File
            생성자를 이용해 생성하고, 이를 URL.createObjectURL메서드를 이용해
            URL로 만든 다음, 지정한 a태그의 href에 연결하는 방식을 사용했습니다.
          </p>
          <p>
            그리고 이러한 업로드와 다운로드를 위한 기능을 함수화시켜 쉽게 사용할
            수 있도록 했습니다. 각 함수는 기능에 맞게 a태그와 input태그를
            document.createElement로 생성했다가 click메서드로 강제로 클릭시키게
            한 다음 문서에서 삭제하는 방식을 사용합니다. 따라서 React의 어떤
            컴포넌트에서든지 해당 함수를 사용하기만 하면 쉽게 파일 다운로드와
            업로드가 가능하도록 했습니다.
          </p>
          <h3>JSON parse</h3>
          <p>
          ...
          <p>
            이러한 이유로 원래 Date객체였던 데이터가 ISO문자열의 형태로 변환되어
            애플리케이션에 적용되는 문제가 발생했고, 이를 해결하기 위해 여러
            방법을 찾아보았습니다.
          </p>
          <p>
            결국 이를 해결하기 위해 사용한 방법은 JSON.parse함수에서 제공하는 두
            번째 매개변수인 reviver을 사용하는 방법이었습니다. reviver에 결과
            값을 원하는 형태로 변환하는 함수를 사용하면, 문자열 형태의
            Date데이터를 실제 Date타입으로 변환할 수 있었습니다.
          </p>
          <p>
            이때 아무런 문자열이나 Date로 변환하면 안되었기 때문에, 정규표현식을
            이용하여 ISO Date 문자열의 형태를 가진 문자열만을 확인해서 이를 new
            Date()로 변형하는 방식을 사용하였습니다.
          </p>
        </section>
      </article>
    </main>
